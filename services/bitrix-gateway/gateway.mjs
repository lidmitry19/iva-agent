...
// The digest needs a fresh authoritative state for a known task, not its
// description, checklist, or discussion. The policy-safe list shape still
// carries the group and role fields required by policy evaluation.
const STATUS_SELECT = Object.freeze([
  "ID", "TITLE", "GROUP_ID", "STATUS", "REAL_STATUS", "RESPONSIBLE_ID",
  "RESPONSIBLE", "ACCOMPLICE", "ACCOMPLICES", "DEADLINE", "CLOSED_DATE",
]);
...
function hasRequiredScopes(scopes) {
  return (
    (scopes.includes("task") || scopes.includes("tasks")) &&
    scopes.includes("im")
  );
}
...
function parseListOptions(options = {}) {
  ...
  if (search.length > 200) {
    throw new GatewayError(
      "INVALID_QUERY",
      "search must contain at most 200 characters.",
      {
        status: 400,
        category: "invalid_request",
      },
...
export class BitrixReadOnlyGateway {
  ...
  async listActiveTasks() {
    const { userId, tasks } = await this.#authorizedTasks({
      status: "active",
      search: "",
    });
  ...
  async #authorizedTasks(query) {
    ...
    tasks.sort(
      (left, right) =>
        String(right.changedAt ?? "").localeCompare(
          String(left.changedAt ?? ""),
  ...
  async taskSnapshot(taskId) {
    ...
    if (task.id !== normalizedTaskId) {
      throw new GatewayError(
        "TASK_ID_MISMATCH",
        "Bitrix returned a different task than requested.",
        {
          status: 502,
    ...
    const checklistResponse = await this.client.request(
  ...
  async taskStatus(taskId) {
    ...
    if (task.id !== normalizedTaskId) {
      throw new GatewayError(
        "TASK_ID_MISMATCH",
        "Bitrix returned a different task than requested.",
        { status: 502, category: "invalid_response" },
      );
    }
    this.policy.assert(task, userId);
    return {
    ...
    if (!this.readyPromise) {
      this.readyPromise = (async () => {
        ...
        if (!userId || !/^[1-9]\d*$/u.test(userId)) {
          throw new GatewayError(
            "PROFILE_INVALID",
            "The Bitrix profile did not return a valid user ID.",
            {
              status: 503,
        ...
        if (!hasRequiredScopes(scopes)) {
          throw new GatewayError(
            "SCOPE_MISSING",
            "The Bitrix webhook lacks one or more required read scopes.",
            {
              status: 503,
              category: "preflight_failed",
            },
  ...
  async #listRole(field, userId, search, status) {
    ...
    for (let page = 0; page < this.maxPages; page += 1) {
      ...
      if (seenStarts.has(startKey)) {
        throw new GatewayError(
          "BITRIX_PAGINATION_INVALID",
          "Bitrix returned a repeated task-list cursor.",
          {
            status: 502,
            category: "invalid_response",
          },
    ...
    throw new GatewayError(
      "BITRIX_PAGINATION_LIMIT",
      "The Bitrix task list exceeded the pagination safety limit.",
      {
        status: 502,
        category: "invalid_response",
      },
  ...
  async #discussion(task) {
    ...
    return {
      ...
      messages: await this.#legacyComments(task.id, resolution),
    };
  }

  async #chatMessages(chatId) {
    if (!this.chatReadVerified) {
      throw new GatewayError(
        "CHAT_READ_STATE_UNVERIFIED",
        "Chat history reads are disabled until read-state behavior is verified.",
        {
          status: 503,
    ...
    for (let page = 0; page < this.maxPages; page += 1) {
      ...
      if (seenCursors.has(cursorKey)) {
        throw new GatewayError(
          "BITRIX_PAGINATION_INVALID",
          "Bitrix returned a repeated discussion cursor.",
          {
            status: 502,
            category: "invalid_response",
          },
      ...
      if (pageMessages.length === 0) {
        throw new GatewayError(
          "BITRIX_PAGINATION_INVALID",
          "Bitrix returned a full discussion page without valid message IDs.",
          {
            status: 502,
            category: "invalid_response",
          },
      ...
      lastId = String(
        ids.reduce((minimum, value) => (value < minimum ? value : minimum)),
      );
    }
    throw new GatewayError(
      "BITRIX_PAGINATION_LIMIT",
      "The Bitrix discussion exceeded the pagination safety limit.",
      {
        status: 502,
        category: "invalid_response",
      },
  ...
  async #legacyComments(taskId, resolution) {
    ...
    if (!Number.isSafeInteger(legacyTaskId) || legacyTaskId < 1) {
      throw new GatewayError(
        "LEGACY_TASK_ID_UNSUPPORTED",
        "The legacy Bitrix comment API requires a safe integer task ID.",
        {
          status: 502,
          category: "invalid_response",
        },
  ...
  #assertTaskId(taskId) {
    ...
    if (!/^[1-9]\d*$/u.test(value) || value.length > 20) {
      throw new GatewayError(
        "INVALID_TASK_ID",
        "Task ID must be a positive numeric Bitrix ID.",
        {
          status: 400,
          category: "invalid_request",
        },
