...
function assertReadStatePreflightParams(method, params) {
  ...
  if (method === "im.dialog.get") {
    if (
      ...
      throw new GatewayError(
        "METHOD_NOT_ALLOWED",
        "Read-state preflight dialog parameters are fixed.",
        {
          status: 403,
  ...
  if (
    ...
    throw new GatewayError(
      "METHOD_NOT_ALLOWED",
      "Read-state preflight message parameters are fixed.",
      {
        status: 403,
        category: "allowlist_denied",
      },
...
function responseTooLarge() {
  return new GatewayError(
    "BITRIX_RESPONSE_TOO_LARGE",
    "Bitrix returned a response larger than the gateway safety limit.",
    {
      status: 502,
...
export class BitrixHttpClient {
  constructor({
    ...
    if (
      ...
      throw new GatewayError(
        "CONFIG_INVALID",
        "BITRIX_WEBHOOK_URL must be an HTTPS URL without embedded credentials.",
        {
          status: 503,
          category: "configuration_error",
        },
    ...
    this.webhookUrl = parsed.href.endsWith("/")
      ? parsed.href
      : `${parsed.href}/`;
    this.chatReadVerified = env?.BITRIX_CHAT_READ_VERIFIED === "true";
    ...
    this.maxRetryWindowMs = Math.max(
      this.maxDelayMs,
      Math.min(
        Number(maxRetryWindowMs) || DEFAULT_MAX_RETRY_WINDOW_MS,
    ...
    this.overloadRetryMs = Math.max(
      this.maxDelayMs,
      Math.min(
        Number(overloadRetryMs) || DEFAULT_OVERLOAD_RETRY_MS,
        this.maxRetryWindowMs,
      ),
    );
    this.maxResponseBytes = Math.max(
      1,
      Math.min(Number(maxResponseBytes) || 8 * 1024 * 1024, 16 * 1024 * 1024),
    );
    this.sleep = sleep;
    this.now = now;
  }
  ...
  async request(method, params = {}, context = {}) {
    if (!READ_METHODS.has(method)) {
      throw new GatewayError(
        "METHOD_NOT_ALLOWED",
        "The requested Bitrix method is not allowed.",
        {
          status: 403,
          category: "allowlist_denied",
        },
    ...
    if (
      ...
      throw new GatewayError(
        "METHOD_NOT_ALLOWED",
        "The Bitrix scope method is limited to gateway preflight.",
        {
          status: 403,
          category: "allowlist_denied",
        },
    ...
    if (method === "im.dialog.get") {
      if (!readStatePreflight) {
        throw new GatewayError(
          "METHOD_NOT_ALLOWED",
          "The Bitrix dialog method is limited to read-state preflight.",
          {
            status: 403,
            category: "allowlist_denied",
          },
  ...
  async getLegacyComments(params, resolution) {
    ...
    if (!authorized) {
      throw new GatewayError(
        "METHOD_NOT_ALLOWED",
        "Legacy task comments are allowed only after chat resolution is exhausted.",
        {
          status: 403,
          category: "allowlist_denied",
        },
  ...
  async #perform(method, params) {
    ...
    for (let attempt = 1; attempt <= this.maxAttempts; attempt += 1) {
      ...
      const denied =
        response.status === 401 ||
        response.status === 403 ||
        /(?:ACCESS|AUTH|SCOPE|PERMISSION)/u.test(code);
      if (denied) {
        throw new GatewayError(
          "BITRIX_ACCESS_DENIED",
          "Bitrix denied this read request.",
          {
            status: 403,
      ...
      const limited =
        response.status === 429 || code === "QUERY_LIMIT_EXCEEDED";
      const serverWindow = serverRetryWindow(
        response,
        this.now(),
        this.maxRetryWindowMs,
      );
      ...
      if (retryable && attempt < this.maxAttempts) {
        if (serverWindow !== null && serverWindow > this.maxDelayMs) {
          throw new GatewayError(
            ...
            limited
              ? "Bitrix is temporarily rate limited."
              : "Bitrix is temporarily unavailable.",
            {
              status: 503,
        ...
        const delay =
          serverWindow ??
          boundedBackoff(attempt, this.baseDelayMs, this.maxDelayMs);
        await this.sleep(delay);
        continue;
      }
      ...
      if (overload) {
        throw new GatewayError(
          ...
          "Bitrix rejected the request because its overload limit was reached.",
          {
            status: 503,
      ...
      if (!response.ok || code) {
        throw new GatewayError(
          ...
          "Bitrix could not complete this read request.",
          {
            status: 502,
      ...
      if (!payload || typeof payload !== "object") {
        throw new GatewayError(
          ...
          "Bitrix returned an invalid response.",
          {
            status: 502,
            category: "invalid_response",
          },
