...
const DOWNLOAD_URL_MARKERS =
  /(?:\/download(?:\/|\?|$)|\/upload\/|\/disk\/|\/files?\/|attachment|download=|file[_-]?id=|auth=|token=)/iu;
...
const STATUS_ALIASES = new Map(
  Object.entries(STATUS_NAMES).map(([code, name]) => [name, Number(code)]),
);
...
export function idString(value) {
  if (value && typeof value === "object")
    value = pick(value, "ID", "USER_ID", "id", "userId");
...
export function safeText(value) {
  ...
  // eslint-disable-next-line no-control-regex -- NUL stripping is an IPC boundary.
...
function personName(raw) {
  ...
  return safeText(
    [pick(raw, "FIRST_NAME"), pick(raw, "LAST_NAME")].filter(Boolean).join(" "),
  );
}
...
function normalizeIdList(raw) {
  const values = Array.isArray(raw)
    ...
    : raw === undefined || raw === null
      ? []
      : [raw];
  return [...new Set(values.map(idString).filter(Boolean))].sort(
    compareCanonicalIds,
  );
}
...
function scalarConflict(raw, scalarNames, objectNames) {
  if (!hasField(raw, ...scalarNames) || !hasField(raw, ...objectNames))
    return false;
...
export function normalizeTask(raw, { portalOrigin = "" } = {}) {
  ...
  const responsible = normalizePerson(
    responsibleObject,
    rawResponsibleId,
    pick(raw, "RESPONSIBLE_NAME"),
  );

  const rawAccompliceIdsPresent = hasField(raw, "ACCOMPLICE", "ACCOMPLICE_IDS");
  const rawAccompliceIds = normalizeIdList(
    pick(raw, "ACCOMPLICE", "ACCOMPLICE_IDS"),
  );
  ...
  const accompliceIds = rawAccompliceIdsPresent
    ? rawAccompliceIds
    : objectAccompliceIds;
  const accompliceNames = new Map(
    accompliceObjects.map((person) => [person.id, person.name]),
  );
  const accomplices = accompliceIds.map((personId) => ({
    id: personId,
    name: accompliceNames.get(personId) ?? "",
  }));

  const creatorId = idString(pick(raw, "CREATED_BY", "CREATED_BY_ID"));
  const creator = normalizePerson(
    pick(raw, "CREATOR", "CREATED_BY_USER"),
    creatorId,
    pick(raw, "CREATED_BY_NAME"),
  );
  const groupId = idString(pick(raw, "GROUP_ID"));
  const commentsCountValue = pick(
    raw,
    "COMMENTS_COUNT",
    "COMMENTSCOUNT",
    "COMMENTS",
  );
  const parsedCommentsCount = Number.parseInt(
    String(commentsCountValue ?? "0"),
    10,
  );
  const chatId = idString(pick(raw, "CHAT_ID", "CHATID"));
  const statusCode = normalizeStatusCode(pick(raw, "STATUS"));
  const realStatusCode =
    normalizeStatusCode(pick(raw, "REAL_STATUS", "REALSTATUS")) || statusCode;
  const closedAt = nullableText(pick(raw, "CLOSED_DATE", "CLOSED_AT"));
  const closed =
    realStatusCode === 5 || realStatusCode === 7 || Boolean(closedAt);
  ...
  const roleFieldsComplete =
    ...
    !(
      rawAccompliceIdsPresent &&
      accompliceObjectsPresent &&
      !sameIds(rawAccompliceIds, objectAccompliceIds)
    );

  let url = "";
  if (portalOrigin && id) {
    try {
      url = new URL(`/workgroups/group/97/tasks/task/view/${id}/`, portalOrigin)
        .href;
    } catch {
      url = "";
    }
  ...
  return {
    ...
    chatId,
    commentsCount:
      Number.isFinite(parsedCommentsCount) && parsedCommentsCount > 0
        ? parsedCommentsCount
        : 0,
    roleFieldsComplete,
  };
  ...
  for (const row of rows) {
    ...
    normalized.push({
      ...
      completed:
        completedRaw === true ||
        completedRaw === 1 ||
        /^(?:1|Y|YES|TRUE)$/iu.test(String(completedRaw ?? "")),
...
function countAttachments(message) {
  ...
  for (const candidate of candidates) {
    if (Array.isArray(candidate)) total += candidate.length;
      ...
      total += Object.keys(candidate).length;
    else if (candidate !== null && candidate !== undefined && candidate !== "")
      total += 1;
  }
  return total;
}
...
export function normalizeChatMessages(response) {
  ...
  for (const row of rows) {
    ...
    const author =
      users.get(authorId) ??
      normalizePerson(pick(row, "AUTHOR"), authorId, pick(row, "AUTHOR_NAME"));
    normalized.push({
      id,
...
export function normalizeLegacyComments(response) {
  ...
  return rows.flatMap((row) => {
    ...
    return [
      {
        id,
        author: normalizePerson(
          pick(row, "AUTHOR"),
          authorId,
          pick(row, "AUTHOR_NAME"),
        ) ?? { id: authorId, name: "" },
...
export function dedupeAndSortMessages(messages) {
  ...
  return [...byId.values()].sort((left, right) => {
    const dateOrder = String(left.createdAt ?? "").localeCompare(
