...
function dialogEvidence(response) {
  ...
  return {
    ...
    complete:
      counter.valid && unreadId.valid && lastId.valid && lastMessageId.valid,
  };
}
...
export async function runReadStatePreflight(
  taskId,
  { client = new BitrixHttpClient(), policy = new BitrixTaskPolicy() } = {},
) {
  ...
  const task = normalizeTask(taskFromGet(taskResponse), {
    portalOrigin: client.portalOrigin,
  });
  ...
  const beforeEvidence = dialogEvidence(
  ...
  const afterEvidence = dialogEvidence(
    await client.request(
      "im.dialog.get",
      { DIALOG_ID: dialogId },
      READ_STATE_CONTEXT,
...
export async function main(argv = process.argv.slice(2)) {
  try {
    ...
    process.stdout.write(
      `${JSON.stringify({ ok: false, result: "error", error_code: errorCode })}\n`,
    );
    return 1;
  }
}
