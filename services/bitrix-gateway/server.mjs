...
function listOptions(url) {
  ...
  if (
    ...
    (!/^\d+$/u.test(options.offset) ||
      !Number.isSafeInteger(Number(options.offset)))
  ) {
    throw new GatewayError(
      "INVALID_QUERY",
    ...
    try {
        ...
        sendJson(
          ...
          405,
          {
            ...
            error: {
              ...
              message: "Only GET is supported by the Bitrix gateway.",
            },
          },
          { allow: "GET" },
        );
      ...
      if (url.pathname === "/v1/tasks/active") {
        ...
        if (url.search !== "") {
          throw new GatewayError(
            "INVALID_QUERY",
            "The daily task endpoint does not accept query parameters.",
            {
              status: 400,
              category: "invalid_request",
            },
      ...
      const statusMatch = /^\/v1\/tasks\/([1-9]\d*)\/status$/u.exec(
        url.pathname,
      );
      if (statusMatch && url.search === "") {
        operation = "task_status";
        taskId = statusMatch[1];
        const result = await gateway.taskStatus(taskId);
        sendJson(response, 200, result);
        logResult(logger, operation, taskId, startedAt, "ok");
        return;
      }
      const snapshotMatch = /^\/v1\/tasks\/([1-9]\d*)\/snapshot$/u.exec(
        url.pathname,
      );
      ...
      sendJson(response, 404, {
        ...
        error: {
          code: "NOT_FOUND",
          message: "The requested Bitrix gateway endpoint does not exist.",
        },
      });
...
async function removeStaleSocket(socketPath) {
  try {
    ...
    if (!details.isSocket())
      throw new Error("Refusing to replace a non-socket gateway path.");
...
export async function startServer({
  ...
  const activeGateway =
    gateway ??
    new BitrixReadOnlyGateway({ client: new BitrixHttpClient({ env }) });
  await mkdir(path.dirname(socketPath), { recursive: true, mode: 0o750 });
  await removeStaleSocket(socketPath);
  const server = http.createServer(
    createRequestHandler({ gateway: activeGateway, logger }),
  );
