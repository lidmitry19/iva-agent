...
function validateTaskId(value: string | number): string {
  ...
  if (!/^[1-9]\d*$/.test(id)) {
    throw new BitrixGatewayError(
      "invalid_task_id",
      "Bitrix task ID must be a positive integer",
    );
  }
  return id;
}
...
export class BitrixGatewayClient {
  ...
  constructor(
    socketPath = process.env.BITRIX_GATEWAY_SOCKET || DEFAULT_SOCKET,
      ...
      const req = request(
        ...
        (res) => {
          ...
          res.on("end", () => {
            ...
            try {
              ...
              reject(
                new BitrixGatewayError(
                  ...
                  "Bitrix gateway returned invalid JSON",
                ),
              );
              return;
            }
            ...
            const errorCode = (errorBody.error?.code || "gateway_error")
              .trim()
              .toLowerCase();
            reject(
              new BitrixGatewayError(
        ...
        reject(
          new BitrixGatewayError(
            ...
            code === "gateway_timeout"
              ? "Bitrix gateway timed out"
              : "Bitrix gateway is unavailable",
          ),
        );
      });
      req.end();
