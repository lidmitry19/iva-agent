...
export interface BitrixSyncState {
  ...
  taskArtifactHash: string | null;
  commentsArtifactHash: string | null;
...
async function setDirectoryModeNoFollow(
  path: string,
  mode: number,
): Promise<void> {
  ...
  const handle = await open(
    path,
    fsConstants.O_RDONLY | DIRECTORY_ONLY | NO_FOLLOW,
  );
  try {
...
async function ensureConfiguredDirectory(
  path: string,
  mode: number,
): Promise<string> {
...
async function ensureContainedDirectory(
  ...
  if (options.mode !== undefined)
    await setDirectoryModeNoFollow(path, options.mode);
  return canonical;
}
...
async function assertContainedRegularFile(
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      const isWindowsDisappearance =
        process.platform === "win32" && code === "EBADF";
      ...
      if (replacement.isSymbolicLink() || !replacement.isFile())
        throw unsafeRepositoryPath();
    ...
    try {
      ...
      try {
        ...
        const parentDisappeared =
          isMissing(parentError) ||
...
async function readContainedTextOrEmpty(
  path: string,
  canonicalRoot: string,
): Promise<string> {
  try {
...
function sortedSnapshot(snapshot: BitrixTaskSnapshot): BitrixTaskSnapshot {
  return {
    task: {
      ...
      accomplices: [...snapshot.task.accomplices].sort((a, b) =>
...
function businessSummary(snapshot: BitrixTaskSnapshot): BusinessSummary {
  return {
    ...
    messageIds: snapshot.discussion.messages
      .map((message) => message.id)
      .sort(compareNumericIds),
  };
}

export function taskDirectoryPath(
  vaultDir: string,
...
export function renderTaskMarkdown(
  ...
  return [
    ...
    ...(task.accomplices.length
      ? task.accomplices.map((person) => `  - ${yaml(person.id)}`)
      : ["  []"]),
...
function historyChanges(
  ...
  if (previous.summary.title !== next.summary.title)
    changes.push(
      `Изменено название: ${short(previous.summary.title)} → ${short(next.summary.title)}.`,
    );
  if (previous.summary.status !== next.summary.status)
    changes.push(
      `Статус изменён: ${previous.summary.status} → ${next.summary.status}.`,
    );
  if (previous.summary.deadline !== next.summary.deadline)
    changes.push(
      `Изменён срок: ${short(previous.summary.deadline)} → ${short(next.summary.deadline)}.`,
    );
  if (previous.summary.responsibleId !== next.summary.responsibleId)
    changes.push(
      `Изменён ответственный: ID ${previous.summary.responsibleId} → ID ${next.summary.responsibleId}.`,
    );
  if (
    stableStringify(previous.summary.accompliceIds) !==
    stableStringify(next.summary.accompliceIds)
  )
    changes.push("Изменён состав соисполнителей.");
  if (previous.summary.descriptionHash !== next.summary.descriptionHash)
    changes.push("Изменено описание задачи.");
  if (previous.summary.checklistHash !== next.summary.checklistHash)
  ...
  else if (previous.discussionHash !== next.discussionHash)
    changes.push("Изменено обсуждение задачи.");
  return changes;
}
...
async function containedFileExists(
  path: string,
  canonicalRoot: string,
): Promise<boolean> {
...
function normalizeBitrixSyncState(
  value: unknown,
  taskId: string,
): BitrixSyncState | null {
  ...
  if (
    ...
    (state.taskArtifactHash !== undefined &&
      state.taskArtifactHash !== null &&
      !isHash(state.taskArtifactHash)) ||
    (state.commentsArtifactHash !== undefined &&
      state.commentsArtifactHash !== null &&
      !isHash(state.commentsArtifactHash)) ||
    ...
    (state.dailyFinalized !== undefined &&
      typeof state.dailyFinalized !== "boolean") ||
    !isBusinessSummary(state.summary)
  )
    return null;
  return {
    ...state,
    taskArtifactHash: state.taskArtifactHash ?? null,
    commentsArtifactHash: state.commentsArtifactHash ?? null,
    dailyFinalized: state.dailyFinalized ?? false,
  } as BitrixSyncState;
}
...
function sameLockOwner(
  left: TaskLockOwner | null,
  right: TaskLockOwner,
): boolean {
  return Boolean(left && left.pid === right.pid && left.token === right.token);
}

async function readTaskLockOwner(
  path: string,
  canonicalRoot: string,
): Promise<TaskLockOwner | null> {
  return parseTaskLockOwner(
    await readContainedTextOrEmpty(path, canonicalRoot),
  );
}
...
async function writeExclusiveLockMarker(
  ...
  try {
    ...
    if (!sameLockOwner(current, owner))
      throw new Error("bitrix_lock_marker_lost");
  } catch (error) {
    await handle?.close().catch(() => undefined);
    throw error;
  }
...
async function removeLockMarkerOwnedBy(
  ...
  const quarantinedState = await lockMarkerIsReapable(
    quarantinePath,
    canonicalRoot,
  );
  if (
...
async function lockMarkerIsReapable(
  path: string,
  canonicalRoot: string,
): Promise<{
  exists: boolean;
  ...
  try {
    ...
    if (
      ...
      try {
        replacement = await assertContainedRegularFile(
          path,
          canonicalRoot,
          true,
        );
...
function sameLockMarkerState(
  left: LockMarkerState,
  right: LockMarkerState,
): boolean {
  return (
    left.exists &&
    right.exists &&
...
async function restoreQuarantinedLockMarker(
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "EEXIST") {
      throw new Error("bitrix_lock_restore_conflict", { cause: error });
    }
    throw error;
  }
...
async function cleanupLockQuarantine(
  lockPath: string,
  canonicalRoot: string,
): Promise<void> {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      const retryable =
        isMissing(error) ||
        (process.platform === "win32" &&
          (code === "EBADF" || code === "EPERM"));
      if (!retryable) throw error;
      const replacement = await pathStat(lockPath);
      if (!replacement) return null;
      if (replacement.isSymbolicLink() || !replacement.isDirectory())
        throw unsafeRepositoryPath();
...
async function tryReapTaskLockOnce(
  lockPath: string,
  canonicalRoot: string,
): Promise<boolean> {
  ...
  if (reaperState.exists) {
    ...
    try {
      ...
      if (
        !sameLockMarkerState(reaperState, quarantinedState) ||
        !quarantinedState.reapable
      ) {
        await restoreQuarantinedLockMarker(
          quarantinePath,
          reaperPath,
          canonicalRoot,
        );
  ...
  const reaperOwner: TaskLockOwner = {
    version: 1,
    pid: process.pid,
    token: randomUUID(),
  };
  try {
    ...
    await removeLockMarkerOwnedBy(reaperPath, reaperOwner, canonicalRoot).catch(
      () => false,
    );
    throw error;
  }
  ...
  try {
    const currentOwnerState = await lockMarkerIsReapable(
      ownerPath,
      canonicalRoot,
    );
    ...
    if (currentOwnerState.exists) {
      ...
      if (
        !sameLockMarkerState(currentOwnerState, quarantinedState) ||
        !quarantinedState.reapable
      ) {
        await restoreQuarantinedLockMarker(
          quarantinePath,
...
async function tryReapTaskLock(
  lockPath: string,
  canonicalRoot: string,
): Promise<boolean> {
...
export class BitrixTaskRepository {
  ...
  async sync(
    rawSnapshot: BitrixTaskSnapshot,
    now = new Date(),
  ): Promise<RepositorySyncResult> {
    const taskId = validateTaskId(rawSnapshot.task.id);
    return await this.runExclusiveTask(taskId, (transaction) =>
      transaction.sync(rawSnapshot, now),
    );
  }
    ...
    return await this.withTaskLock(taskId, async () => {
      ...
      return await operation({
        ...
        sync: async (rawSnapshot, now = new Date()) => {
          ...
          if (validateTaskId(snapshot.task.id) !== taskId)
            throw new Error("bitrix_task_id_mismatch");
  ...
  private async syncUnlocked(
    ...
    const renderedTask = renderTaskMarkdown(snapshot, syncedAt);
    const renderedComments = renderCommentsMarkdown(snapshot);
    const state: BitrixSyncState = {
      version: 1,
      taskId,
      taskHash,
      discussionHash,
      taskArtifactHash: hash(renderedTask),
      commentsArtifactHash: hash(renderedComments),
      lastMessageId:
        snapshot.discussion.messages
          .map((message) => message.id)
          .sort(compareNumericIds)
          .at(-1) ?? null,
    ...
    await Promise.all(
      [taskPath, commentsPath, historyPath].map((path) =>
        setRegularFileModeNoFollow(path, roots.tasksRoot, 0o600),
      ),
    );
    const artifactsMatch =
      markdownExists &&
      previous !== null &&
      previous.taskArtifactHash !== null &&
      previous.commentsArtifactHash !== null &&
      hash(await readContainedText(taskPath, roots.tasksRoot)) ===
        previous.taskArtifactHash &&
      hash(await readContainedText(commentsPath, roots.tasksRoot)) ===
        previous.commentsArtifactHash;
    const unchanged =
      artifactsMatch &&
      historyExists &&
      previous?.taskHash === taskHash &&
      previous.discussionHash === discussionHash;
    if (unchanged) {
      await this.writeState({
        ...state,
        taskArtifactHash: previous.taskArtifactHash,
        commentsArtifactHash: previous.commentsArtifactHash,
      });
      return { outcome: "unchanged", taskId, syncedAt, taskDir };
    }

    await atomicWrite(taskPath, renderedTask, roots.tasksRoot);
    await atomicWrite(commentsPath, renderedComments, roots.tasksRoot);
    const changes = historyChanges(previous, state, !localArtifactsExist);
    if (changes.length > 0) {
      const existingHistory = await readContainedTextOrEmpty(
        historyPath,
        roots.tasksRoot,
      );
      ...
      if (!existingHistory.includes(marker)) {
        await atomicWrite(
          historyPath,
          appendHistory(existingHistory, syncedAt, changes, marker),
          roots.tasksRoot,
        );
      }
    ...
    return {
      outcome: previous || localArtifactsExist ? "updated" : "created",
      taskId,
      syncedAt,
      taskDir,
    };
  ...
  async recordFailure(
    ...
    const taskId = validateTaskId(taskIdValue);
    await this.runExclusiveTask(taskId, (transaction) =>
      transaction.recordFailure(code, retryAt),
    );
  }
  ...
  async search(query: string, limit = 20): Promise<BitrixLocalSearchHit[]> {
    ...
    for (const entry of entries) {
      ...
      const snippet = text
        .slice(start, at + needle.length + 140)
        .replace(/\s+/g, " ")
        .trim();
  ...
  async readState(
    taskIdValue: string | number,
  ): Promise<BitrixSyncState | null> {
    const taskId = validateTaskId(taskIdValue);
    const roots = await this.ensurePrivateRoots();
    try {
      const parsed = JSON.parse(
        await readContainedText(
          join(this.stateRoot, `${taskId}.json`),
          roots.stateRoot,
        ),
      ) as unknown;
  ...
  private async ensurePrivateRoots(): Promise<PrivateRoots> {
    ...
    await ensureContainedDirectory(tasksDir, vaultRoot, {
      create: true,
      mode: 0o700,
    });
    ...
    const stateRoot = await ensureContainedDirectory(this.stateRoot, dataRoot, {
      create: true,
      mode: 0o700,
    });
  ...
  private async withTaskLock<T>(
    ...
    while (!acquired) {
      ...
      try {
        ...
        if (!(await validateTaskLockDirectory(lockPath, roots.stateRoot)))
          continue;
        if (await tryReapTaskLock(lockPath, roots.stateRoot)) continue;
        if (Date.now() >= deadline)
          throw new Error("bitrix_sync_busy", { cause: error });
      ...
      if (createdInfo.isSymbolicLink() || !createdInfo.isDirectory())
        throw unsafeRepositoryPath();
      const validatedInfo = await validateTaskLockDirectory(
        lockPath,
        roots.stateRoot,
      );
      ...
      try {
        ...
        if (
          await taskLockDirectoryChanged(
            lockPath,
            roots.stateRoot,
            validatedInfo,
          )
    ...
    try {
      ...
      await releaseTaskLock(lockPath, acquired, roots.stateRoot).catch(
        () => undefined,
      );
    }
  }
