...
const MESSAGES: Record<string, string> = {
  ...
  not_participant:
    "Вы не являетесь исполнителем или соисполнителем этой задачи.",
  policy_incomplete:
    "Bitrix вернул неполные данные; доступ отклонён безопасным образом.",
  ...
  discussion_unavailable:
    "Обсуждение задачи недоступно через разрешённые read-only методы.",
  ...
  chat_read_state_unverified:
    "Чтение чата задачи отключено до подтверждения, что API не меняет состояние прочитанности.",
  ...
  policy_data_incomplete:
    "Bitrix вернул неполные данные; доступ отклонён безопасным образом.",
  scope_missing:
    "У webhook нет необходимых прав для безопасного чтения задач и обсуждений.",
  bitrix_rate_limited: "Bitrix временно ограничил частоту запросов.",
  bitrix_overload_limit:
    "Bitrix временно ограничил нагрузку; использую последний снимок.",
...
export function safeBitrixError(error: unknown): {
  ...
  const code =
    error instanceof Error && error.message === "bitrix_sync_busy"
      ? "sync_busy"
      : "unexpected_error";
  return {
    ok: false,
    error: {
      ...
      message:
        code === "sync_busy"
          ? "Эта задача уже синхронизируется; повторите позже."
          : "Неожиданная безопасная ошибка Bitrix-интеграции.",
    },
  };
}
