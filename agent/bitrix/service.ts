...
export function isTransientBitrixError(error: unknown): boolean {
  return (
    ...
    TRANSIENT_CODES.has(error.code.toLowerCase())
  );
}

export class BitrixTaskService {
  private readonly gateway: BitrixGatewayClient;
  private readonly repository: BitrixTaskRepository;

  constructor(
    gateway = new BitrixGatewayClient(),
    repository = new BitrixTaskRepository(),
  ) {
  ...
  async syncTask(taskIdValue: string | number): Promise<RepositorySyncResult> {
    ...
    return await this.repository.runExclusiveTask(
      taskId,
      async (transaction) => {
        const persistedRetryAt =
          transaction.state?.nextAllowedAttemptAt ?? null;
        const persistedRetryAtMs = persistedRetryAt
          ? Date.parse(persistedRetryAt)
          : Number.NaN;
        if (
          persistedRetryAt &&
          Number.isFinite(persistedRetryAtMs) &&
          persistedRetryAtMs > Date.now()
        ) {
  ...
  async readTask(taskIdValue: string | number): Promise<{
    ...
    try {
      ...
      if (staleReason)
        throw new BitrixGatewayError(
          staleReason,
          "Bitrix is unavailable and no local snapshot exists",
        );
      throw error;
    }
    return {
  ...
  async searchLocalTasks(
    query: string,
    limit = 20,
  ): Promise<BitrixLocalSearchHit[]> {
  ...
  async syncDaily(concurrency = 3): Promise<{
    ...
    const states = new Map(
      (await this.repository.listStates()).map((state) => [
        state.taskId,
        state,
      ]),
    );
    ...
    return {
      ...
      failed: results
        .filter(
          (
            result,
          ): result is { taskId: string; outcome: "failed"; code: string } =>
