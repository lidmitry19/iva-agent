...
test("past the depth ceiling the wrapper is gone and the data is not", () => {
  ...
  const unsafeCode = "[click](javascript:alert(1))";
  let deepPreBlock: unknown = {
    type: "pre",
    text: unsafeCode,
    language: "js",
  };
  for (let i = 0; i < MAX_RICH_MESSAGE_DEPTH + 2; i += 1) {
    deepPreBlock = { type: "blockquote", blocks: [deepPreBlock] };
  }
  const deepPre = readRichMessage({
    blocks: [deepPreBlock],
  });
  assert.ok(deepPre.text.includes(unsafeCode));
  assert.deepEqual(scanLinks(deepPre.text), []);
});
