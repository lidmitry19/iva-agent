...
| `OPENCODE_VISION_MODEL`     | `minimax-m3`              | То же для OpenCode Go, bare ID. Берите из тех моделей, что реально читают картинки — многие модели Go отвечают на фото 400; быстрая альтернатива — `qwen3.7-plus`.                                                                                                                                                                                                                                 |
...
| `THINKING_EFFORT`           | _(пусто)_                 | Усилие размышлений: `minimal`, `low`, `medium`, `high`, `xhigh`, `max`. Пишется командами `/model` и `/think` в Telegram ([cli.md](../cli.md)), не руками. `codex` получает его нативно; `ollama`/`opencode` шлют `low`/`medium`/`high` по OpenAI-совместимому протоколу, остальное игнорируют; при выборе `openrouter` визард строку убирает вовсе, а вписанное руками значение там игнорируется. |

Ровно эти четыре имени, буква в букву. Любое другое значение, включая опечатку и другой регистр, останавливает агента на старте и печатает список принятых имён, вместо того чтобы запустить Ollama под именем, которого никто не задавал. То же самое скажут `iva doctor` и экран «📊 Статус» в `/menu` ([troubleshooting.md](../troubleshooting.md)). Если строки `MODEL_PROVIDER` нет вовсе, это по-прежнему `ollama`.
