...
Дополнение 2026-09-01. Исключение для класса «потерян вход владельца»
(`telegram-acceptance:<chat>` в `scripts/poller/routing.ts`): входящее сообщение не дошло
до модели. Такой Alert повторяется раз в 10 минут, а не раз в неделю. Потерянный вход —
прямой ущерб, не шум; неделя молчания выглядит как мёртвый бот. Все остальные Alert
оставляют недельный период без изменений.
