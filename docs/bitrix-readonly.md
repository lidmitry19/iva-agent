...
Before this window, a trusted administrator or root-controlled configuration
channel must provision the reviewed `admin-install.sh` as
`/usr/local/lib/iva-bitrix-admin/install`, with both the helper and its
`iva-bitrix-admin` directory owned by `root:root` at mode `0700`. This
bootstrap/update is deliberately out of band: never install or execute a
script directly from the `iva`-owned checkout with `sudo`. The fixed helper
snapshots every manifest input into a root-only `0700` staging tree, verifies
that snapshot, and executes only the staged transaction.
...
iva_guard_recover_app() {
  ...
  "$NODE24" --check scripts/telegram-poll.ts || return 1
...
"$NODE24" --check scripts/telegram-poll.ts
...
# Open and positively probe this shell's ticket, then invoke only the fixed
# root-owned helper. It collects a missing/invalid secret through the same TTY;
# never put the webhook in shell history, argv, chat, the IVA .env, or an
# iva-readable staging file.
iva_guard_open_sudo_window
/usr/bin/sudo /usr/local/lib/iva-bitrix-admin/install \
  "$EXPECTED_AUDITED_COMMIT"
iva_guard_finish 0
```

The helper reuses a valid protected env. Gateway code is copied into an immutable
root-owned release directory and promoted through an atomic `current` link. The
transaction snapshots the previous link, unit bytes, enablement, and active
state before mutation. An install, restart, socket, or health failure restores
that snapshot and removes only a newly created failed release. If rollback or
the outer guard reports failure, leave IVA's timer disabled and preserve the
reported root-only recovery evidence for inspection.
...
/usr/bin/sudo -u iva-bitrix /usr/bin/node --env-file=/etc/iva-bitrix/bitrix.env \
  /usr/local/lib/iva-bitrix-gateway/current/preflight-read-state.mjs \
...
iva_guard_begin "$IVA_SUDO_STATE"
iva_guard_open_sudo_window
/usr/bin/sudo /usr/local/lib/iva-bitrix-gateway/current/audit-secret.py \
...
`iva doctor` inventories these optional units but never enables or starts them;
only the explicit post-gate commands below may activate the timer.
...
/usr/bin/systemctl --user cat iva-bitrix-sync.service |
  /usr/bin/grep -Fx "ExecStart=$NODE24_BIN_DIR/node --env-file=.env scripts/bitrix-sync.ts --daily"
