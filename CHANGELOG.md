...
- 🧭 **Ива переехала на eve 0.47.3** - сообщения теперь привязаны к активной сессии по `sessionId`, поэтому Stop, `/stop` и `/new` не зависят от старого continuation-механизма. При обновлении контексты диалогов сброшены, память (vault) цела.
