...
async function main(): Promise<void> {
  if (!BOT || !CHAT) throw new Error("TELEGRAM_BOT_TOKEN and TELEGRAM_DIGEST_CHAT_ID are required");
  markNightJob(DATA_DIR, "digest", { targetDate, timeZone: TIME_ZONE, state: "running", artifacts: [receiptPath] });
  ...
  if (night?.jobs.memoryRollup.state !== "success") {
    ...
    const manifest = markNightJob(DATA_DIR, "digest", { targetDate, timeZone: TIME_ZONE, state: "failed", artifacts: [receiptPath], error: `ordinary digest blocked: ${reason}` });
    await notifyNightHealth(DATA_DIR, manifest, "digest");
    await sendDiagnostic(`⚠️ IVA morning digest for ${targetDate} was not sent: ${reason}. Check /status and iva doctor.`);
    process.exitCode = 1;
    return;
  }

  const bitrixAvailable = night.jobs.bitrixSync.state === "success";
  const client = new Client({ host: HOST, ...(BEARER ? { auth: { bearer: () => Promise.resolve(BEARER) } } : {}) });
  ...
  if (result.status === "failed" || !result.message) throw new Error(`agent did not return a digest (status=${result.status})`);
  const delivered = await sendTelegramHtml(BOT, CHAT, result.message, { trace: { session: response.sessionId, source: "digest" } });
  if (!delivered.ok) throw new Error(`Telegram send failed: ${delivered.error}`);
  mkdirSync(join(DATA_DIR, "night-health"), { recursive: true, mode: 0o700 });
  writeFileAtomicSync(receiptPath, `${JSON.stringify({ targetDate, runId: night.runId, deliveredAt: new Date().toISOString(), mode: bitrixAvailable ? "full" : "limited" })}\n`);
  const manifest = markNightJob(DATA_DIR, "digest", {
    targetDate, timeZone: TIME_ZONE, state: "success",
    artifacts: [receiptPath, nightManifestPath(DATA_DIR, targetDate)],
    mode: bitrixAvailable ? "full" : "limited",
  });
  await notifyNightHealth(DATA_DIR, manifest, "digest");
  console.log(`Digest sent to Telegram (runId=${night.runId}, targetDate=${targetDate}, mode=${bitrixAvailable ? "full" : "limited"}).`);
}

main().catch(async (error: unknown) => {
  const manifest = markNightJob(DATA_DIR, "digest", { targetDate, timeZone: TIME_ZONE, state: "failed", artifacts: [receiptPath], error });
  await notifyNightHealth(DATA_DIR, manifest, "digest");
