...
// Current production follows the maintainer-owned fork; historical installs may
// still use the canonical upstream aliases. Keep this an exact GitHub allowlist.
const TRUSTED_REPO_PATHS = new Set([
  "smixs/iva-agent",
  "smixs/iva",
  "lidmitry19/iva-agent",
]);
...
export function validateOfficialCheckout(inputRoot: string): Checkout {
  ...
  if (!isOfficialIvaOrigin(origin))
    throw new Error("origin is not a trusted Iva GitHub repository");
  return { root, origin };
}
