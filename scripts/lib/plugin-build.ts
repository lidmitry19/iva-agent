...
/**
 * What eve 0.47.3 accepts as a connection file name (its CONNECTION_SLUG_PATTERN):
 ...
 * the installed package on 2026-08-31.
 */
 ...
 * declaration up the tree and finds the version's own eve (verified on 0.47.3 on
 * 2026-08-31).
 */
const PLUGIN_INSTALL = ["--omit=dev", "--omit=peer", "--no-audit", "--no-fund"];
const LOCKFILES = ["package-lock.json", "npm-shrinkwrap.json"];
/**
 * The lowercase subset of eve 0.47.3's EXTENSION_SLUG_PATTERN. Agent Plugin names are
 * lowercase already. Verified against the installed package on 2026-08-31.
 */
...
export function mountSource(plugin: CodePlugin): string {
  // The trailing `/.` is load-bearing on eve 0.47.3. Without it, the authored asset
 ...
 * the generated mount (verified on eve 0.47.3 on 2026-08-31). Pointing the mount straight at
