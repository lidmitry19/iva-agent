...
const { BitrixGatewayError, DEFAULT_GATEWAY_TIMEOUT_MS } =
  await import("../../agent/bitrix/gateway-client.ts");
const { BitrixTaskRepository } =
  await import("../../agent/bitrix/repository.ts");
const { BitrixTaskService, isTransientBitrixError } =
  await import("../../agent/bitrix/service.ts");
const { safeBitrixError } = await import("../../agent/bitrix/runtime.ts");
import type { BitrixGatewayClient } from "../../agent/bitrix/gateway-client.js";
...
void test("the production install uses Node 24's built-in TypeScript runner", async () => {
  ...
  const unit = await readFile(
    new URL("../../deploy/iva-bitrix-sync.service", import.meta.url),
    "utf8",
  );

  assert.equal(packageJson.engines?.node, "24.x");
  assert.equal(
    packageJson.scripts?.["bitrix:sync"],
    "node --env-file-if-exists=.env scripts/bitrix-sync.ts --daily",
  );
  assert.equal(packageJson.dependencies?.tsx, undefined);
  assert.equal(packageJson.devDependencies?.tsx, undefined);
  assert.equal(packageLock.packages[""].dependencies?.tsx, undefined);
...
void test("read falls back to a dated snapshot only for transient failures", async (t) => {
  ...
  let error: InstanceType<typeof BitrixGatewayError> | null = null;
  const gateway = {
    getSnapshot: () => {
      if (error) return Promise.reject(error);
      return Promise.resolve(structuredClone(snapshot));
    },
    listTasks: () => Promise.resolve({ userId: "1274", tasks: [], total: 0 }),
    health: () =>
      Promise.resolve({
        ready: true,
        userId: "1274",
...
void test("only sidecar unavailability and rate limits are transient", () => {
  assert.equal(
    isTransientBitrixError(
      new BitrixGatewayError("BITRIX_UNAVAILABLE", "offline"),
    ),
    true,
  );
  assert.equal(
    isTransientBitrixError(
      new BitrixGatewayError("BITRIX_RATE_LIMITED", "limited"),
    ),
    true,
  );
  assert.equal(
    isTransientBitrixError(
      new BitrixGatewayError("BITRIX_OVERLOAD_LIMIT", "busy"),
    ),
    true,
  );
  for (const code of [
    ...
    assert.equal(
      isTransientBitrixError(new BitrixGatewayError(code, "blocked")),
      false,
    );
  }
});

void test("invalid task id is rejected before any gateway call", async () => {
  let calls = 0;
  const gateway = {
    getSnapshot: () => Promise.resolve((calls++, snapshot)),
  } as unknown as BitrixGatewayClient;
  const service = new BitrixTaskService(
    gateway,
    new BitrixTaskRepository("vault", "data"),
  );
  await assert.rejects(service.syncTask("../42"), /positive integer/);
  assert.equal(calls, 0);
});

void test("gateway IPC timeout leaves room for bounded sidecar retries", () => {
  assert.equal(DEFAULT_GATEWAY_TIMEOUT_MS, 90_000);
});

void test("persisted future backoff skips gateway while expired and invalid timestamps proceed", async (t) => {
...
void test("sidecar error codes are lowercased and mapped to safe public messages", () => {
  const codes = [
    "CHAT_READ_STATE_UNVERIFIED",
    "TASK_OUTSIDE_GROUP",
    "TASK_NOT_AUTHORIZED",
  ...
  for (const code of codes) {
    ...
    assert.notEqual(
      ...
      "Не удалось прочитать данные Bitrix безопасным способом.",
    );
    assert.doesNotMatch(publicError.error.message, /unsafe upstream detail/u);
  }
});

void test("daily sync finalizes declined and closedAt tasks once and skips newly discovered closed tasks", async (t) => {
  ...
  await repository.sync(
    taskSnapshot("47", { status: "declined", realStatus: 7, closed: true }),
  );
  ...
  const current = new Map<string, BitrixTaskSnapshot>([
    ...
    [
      "43",
      taskSnapshot("43", { status: "declined", realStatus: 7, closed: true }),
    ],
    [
      "44",
      taskSnapshot("44", {
    ...
    [
      "45",
      taskSnapshot("45", { status: "declined", realStatus: 7, closed: true }),
    ],
    ["46", taskSnapshot("46")],
    [
      "47",
  ...
  const gateway = {
    ...
    listTasks: () =>
      Promise.reject(
        new Error("daily sync must not use the paged public list"),
      ),
    getSnapshot: (taskId: string | number) => {
      const id = String(taskId);
      snapshotCalls.push(id);
      const value = current.get(id);
      if (!value) return Promise.reject(new Error("missing fixture"));
