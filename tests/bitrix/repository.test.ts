...
const {
  ...
  renderTaskMarkdown,
  taskDirectoryPath,
} = await import("../../agent/bitrix/repository.ts");
...
function fixture(): BitrixTaskSnapshot {
  return {
    task: {
      ...
      closedAt: null,
      checklist: [
        { id: "1", title: "Проверить цифры", completed: false, parentId: null },
      ],
    },
    discussion: {
...
void test("numeric task path cannot traverse outside the Bitrix task root", () => {
  assert.equal(
    taskDirectoryPath("/vault", "12345"),
    join("/vault", "tasks", "bitrix", "12345"),
  );
  assert.throws(
    () => taskDirectoryPath("/vault", "../secret"),
    /positive integer/,
  );
  assert.throws(() => taskDirectoryPath("/vault", "01"), /positive integer/);
});

void test("markdown is deterministic and preserves prompt injection only as untrusted text", () => {
  const snapshot = fixture();
  const at = "2026-07-16T06:00:00.000Z";
  assert.equal(
    renderTaskMarkdown(snapshot, at),
    renderTaskMarkdown(structuredClone(snapshot), at),
  );
...
void test("repeated and concurrent syncs are idempotent and history changes once", async (t) => {
  ...
  const before = await Promise.all(
    ["task.md", "comments.md", "history.md"].map((file) =>
  ...
  assert.equal(
    concurrent.every(({ outcome }) => outcome === "unchanged"),
    true,
  );
  ...
  assert.equal(
    (await repository.sync(changed, new Date("2026-07-16T08:00:00.000Z")))
      .outcome,
    "updated",
  );
  ...
  assert.equal(
    await readFile(join(dir, "history.md"), "utf8"),
    historyAfterChange,
  );
});

void test("unchanged source repairs locally modified rendered artifacts", async (t) => {
  const root = await mkdtemp(join(tmpdir(), "iva-bitrix-artifact-repair-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  const vault = join(root, "vault");
  const repository = new BitrixTaskRepository(vault, join(root, "data"));
  const snapshot = fixture();
  await repository.sync(snapshot, new Date("2026-07-16T06:00:00.000Z"));

  const dir = taskDirectoryPath(vault, snapshot.task.id);
  const taskPath = join(dir, "task.md");
  const commentsPath = join(dir, "comments.md");
  ...
  await writeFile(
    taskPath,
    originalTask.replace(
      /^synced_at: .*$/mu,
      'synced_at: "2026-01-01T00:00:00.000Z"',
    ),
  );

  const repairedAt = new Date("2026-07-16T07:00:00.000Z");
  assert.equal(
    (await repository.sync(snapshot, repairedAt)).outcome,
    "updated",
  );
  assert.equal(
    await readFile(taskPath, "utf8"),
    renderTaskMarkdown(snapshot, repairedAt.toISOString()),
  );
  assert.equal(await readFile(historyPath, "utf8"), historyBefore);

  await writeFile(commentsPath, "# forged discussion\n");
  assert.equal(
    (await repository.sync(snapshot, new Date("2026-07-16T08:00:00.000Z")))
      .outcome,
    "updated",
  );
  assert.equal(
    await readFile(commentsPath, "utf8"),
    renderCommentsMarkdown(snapshot),
  );
  assert.equal(await readFile(historyPath, "utf8"), historyBefore);

  assert.equal(
    (await repository.sync(snapshot, new Date("2026-07-16T09:00:00.000Z")))
      .outcome,
    "unchanged",
  );
});

void test("corrupted state is rebuilt without deleting markdown", async (t) => {
  ...
  assert.match(
    await readFile(
      join(root, "vault", "tasks", "bitrix", "12345", "task.md"),
      "utf8",
    ),
    /Power BI/,
  );
...
void test("repository persists the canonical closed flag rather than inferring status 5", async (t) => {
...
void test("recordFailure waits for the per-task lock before updating state", async (t) => {
  ...
  const failure = repository
    .recordFailure("12345", "bitrix_rate_limited", "2026-07-17T00:00:00.000Z")
    .then(() => {
      settled = true;
    });
...
void test("two contenders recover one stale lock without overlapping", async (t) => {
  ...
  const contender = () =>
    repository.runExclusiveTask("12345", async () => {
      active += 1;
      enteredCount += 1;
...
void test("an in-flight stale creator cannot acquire or delete a replacement owner lock", async (t) => {
  ...
  const markerWriteStartedPromise = new Promise<void>((resolve) => {
    markerWriteStarted = resolve;
  });
  let resumeFirstMarker!: () => void;
  const resumeFirstMarkerPromise = new Promise<void>((resolve) => {
    resumeFirstMarker = resolve;
  });
  ...
  fileHandlePrototype.writeFile = async function (
    ...
    const text =
      typeof args[0] === "string"
        ? args[0]
        : Buffer.from(args[0]).toString("utf8");
  ...
  const firstResult = repository
    .runExclusiveTask("12345", () => {
      firstEntered = true;
  ...
  const releaseReplacementPromise = new Promise<void>((resolve) => {
    releaseReplacement = resolve;
  });
  let replacementEntered!: () => void;
  const replacementEnteredPromise = new Promise<void>((resolve) => {
  ...
  const third = repository.runExclusiveTask("12345", () => {
    thirdEntered = true;
    return Promise.resolve();
  });
  await new Promise((resolve) => setTimeout(resolve, 50));
...
void test("an in-flight stale reaper cannot strand its quarantine or block recovery", async (t) => {
  ...
  const resumeStaleMarkerPromise = new Promise<void>((resolve) => {
    resumeStaleMarker = resolve;
  });
  let staleQuarantineRead!: () => void;
  const staleQuarantineReadPromise = new Promise<void>((resolve) => {
    staleQuarantineRead = resolve;
  });
  let resumeQuarantineRead!: () => void;
  const resumeQuarantineReadPromise = new Promise<void>((resolve) => {
    resumeQuarantineRead = resolve;
  });
  ...
  fileHandlePrototype.readFile = async function (
    ...
    const result = await Reflect.apply(originalReadFile, this, args);
...
void test("release cannot remove a replacement owner created after its ownership read", async (t) => {
  ...
  const operationEnteredPromise = new Promise<void>((resolve) => {
    operationEntered = resolve;
  });
  let releaseOperation!: () => void;
  const releaseOperationPromise = new Promise<void>((resolve) => {
  ...
  let ownerRead!: () => void;
  const ownerReadPromise = new Promise<void>((resolve) => {
    ownerRead = resolve;
  });
...
void test("configured roots reject a symlinked ancestor before recursive creation", async (t) => {
  ...
  await assert.rejects(
    repository.sync(fixture()),
    /bitrix_repository_unsafe_path/,
  );
  await assert.rejects(lstat(join(outside, "nested")), { code: "ENOENT" });
});

void test("tasks/bitrix and numeric task directories reject symlink escapes", async (t) => {
...
void test("markdown, state, and lock symlinks are rejected before access", async (t) => {
  ...
  if (
    ...
    return;
  await assert.rejects(
    repository.runExclusiveTask("12345", () => Promise.resolve(undefined)),
    /bitrix_repository_unsafe_path/,
  );
});
