...
test("known chat preflight authorizes first, reads exactly one message, and emits state-only evidence", async () => {
  ...
  const client = new ScriptedClient(
    ({ method, params, context: callContext }) => {
  ...
  assert.doesNotMatch(
    serialized,
    /unit-test-token|Injected task|Dialog title|Dialog text|Message body|Message author/iu,
  );
  assert.deepEqual(Object.keys(result.before), [
    "counter",
    "unread_id",
    "last_id",
    "last_message_id",
  ]);
  assert.deepEqual(Object.keys(result.after), [
    "counter",
    "unread_id",
    "last_id",
    "last_message_id",
...
test("changed dialog evidence compares only the four allowed state fields", async () => {
  ...
  assert.equal(
    Object.prototype.hasOwnProperty.call(result.before, "title"),
    false,
  );
});
...
test("null unread ID is non-discriminating and does not read a message", async () => {
  ...
  assert.equal(
    client.calls.some((call) => call.method === "im.dialog.messages.get"),
    false,
  );
  assert.equal(
    client.calls.filter((call) => call.method === "im.dialog.get").length,
    1,
  );
});
  ...
  for (const scenario of scenarios) {
    await t.test(scenario.name, async () => {
      const client = new ScriptedClient(({ method }) => {
        ...
        if (ready) return ready;
        if (method === "tasks.task.get")
          return { result: { task: scenario.task } };
        throw new Error(`Policy should deny before ${method}`);
      });
      await assert.rejects(runReadStatePreflight("123", { client }), {
        code: scenario.code,
      });
      assert.equal(
        client.calls.some((call) => call.method.startsWith("im.")),
        false,
      );
    });
  }
...
test("chat resolution tries TASKS_TASK then TASKS and returns no_task_chat without a dialog read", async () => {
  const client = new ScriptedClient(({ method, params }) => {
    ...
    if (ready) return ready;
    if (method === "tasks.task.get")
      return { result: { task: rawTask({ CHAT_ID: null }) } };
  ...
  assert.deepEqual(
    client.calls
      .filter((call) => call.method === "im.chat.get")
      .map((call) => call.params.ENTITY_TYPE),
    ["TASKS_TASK", "TASKS"],
  );
  assert.equal(
    client.calls.some((call) => call.method.startsWith("im.dialog.")),
    false,
  );
});
...
test("installer deploys the operator CLI while the guarded runbook uses only the immutable release", async () => {
  const installer = await readFile(
    new URL("../../services/bitrix-gateway/deploy/install.sh", import.meta.url),
    "utf8",
  );
  const runbook = await readFile(
    new URL("../../docs/bitrix-readonly.md", import.meta.url),
    "utf8",
  );
  const preflightSource = await readFile(
    new URL(
      "../../services/bitrix-gateway/preflight-read-state.mjs",
      import.meta.url,
    ),
    "utf8",
  );
  assert.match(
    installer,
    /install -o root -g root -m 0755[\s\\]+[\s\S]*preflight-read-state\.mjs/iu,
  );
  assert.match(
    runbook,
    /iva_guard_open_sudo_window[\s\S]*\/usr\/bin\/sudo -u iva-bitrix \/usr\/bin\/node --env-file=\/etc\/iva-bitrix\/bitrix\.env[\s\S]*\/usr\/local\/lib\/iva-bitrix-gateway\/current\/preflight-read-state\.mjs[\s\\]+[\s\S]*<authorized-task-id-with-chat>/iu,
  );
  assert.doesNotMatch(
    installer,
    /sudo -u iva-bitrix|<authorized-task-id-with-chat>/iu,
  );
