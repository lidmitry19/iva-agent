...
test("policy allows current profile only as responsible or accomplice in group 97", () => {
  ...
  assert.deepEqual(policy.evaluate(responsible, "42"), {
    allowed: true,
    role: "responsible",
  });

  const accomplice = normalizeTask(
    rawTask({
      RESPONSIBLE_ID: "99",
  ...
  assert.deepEqual(policy.evaluate(accomplice, "42"), {
    allowed: true,
    role: "accomplice",
  });
  assert.equal(policy.evaluate(accomplice, "77").code, "NOT_PARTICIPANT");
  assert.equal(
    policy.evaluate(normalizeTask(rawTask({ GROUP_ID: "98" })), "42").code,
    "WRONG_GROUP",
  );
});
...
test("policy fails closed when any role field is missing or contradictory", () => {
  ...
  assert.equal(
    policy.evaluate(normalizeTask(missingAccomplices), "42").code,
  ...
  assert.equal(
    policy.evaluate(normalizeTask(missingResponsible), "43").code,
    "INCOMPLETE_ROLE_FIELDS",
  );

  const contradictory = normalizeTask(
    rawTask({ RESPONSIBLE: { ID: "77", NAME: "Mismatch" } }),
  );
  assert.equal(
    policy.evaluate(contradictory, "42").code,
    "INCOMPLETE_ROLE_FIELDS",
  );

  const contradictoryAccomplices = normalizeTask(
    rawTask({
      ACCOMPLICE: ["43"],
  ...
  assert.equal(
    policy.evaluate(contradictoryAccomplices, "42").code,
    "INCOMPLETE_ROLE_FIELDS",
  );
});
