...
test("runtime allowlist rejects writes, generic legacy comments, and non-preflight scope before fetch", async () => {
  ...
  await assert.rejects(client.request("tasks.task.update", { taskId: "123" }), {
    code: "METHOD_NOT_ALLOWED",
  });
  await assert.rejects(
    client.request("task.commentitem.add", { taskId: "123" }),
    { code: "METHOD_NOT_ALLOWED" },
  );
  await assert.rejects(
    client.request("im.dialog.read", { DIALOG_ID: "chat1" }),
    { code: "METHOD_NOT_ALLOWED" },
  );
  await assert.rejects(
    client.request("im.dialog.unread", { DIALOG_ID: "chat1" }),
    { code: "METHOD_NOT_ALLOWED" },
  );
  await assert.rejects(
    client.request("task.commentitem.getlist", { TASKID: "123" }),
    { code: "METHOD_NOT_ALLOWED" },
  );
  await assert.rejects(client.request("scope"), { code: "METHOD_NOT_ALLOWED" });
  await assert.rejects(
    client.getLegacyComments(
      { TASKID: "123" },
      {
        taskChatIdNull: true,
        tasksTaskNull: true,
...
test("legacy comments require the complete, positive chat-resolution gate", async () => {
  ...
  await client.getLegacyComments(
    { TASKID: "123" },
    {
      taskChatIdNull: true,
      tasksTaskNull: true,
...
test("chat-history reads fail closed until the verification gate is true", async () => {
  let fetchCalls = 0;
  const client = clientWith(async () => {
    fetchCalls += 1;
  ...
  await assert.rejects(
    client.request("im.dialog.messages.get", { DIALOG_ID: "chat1" }),
...
test("verified chat-history reads remain read-only", async () => {
  let fetchCalls = 0;
...
test("read-state preflight bypass requires the exact context and fixed dialog parameters", async () => {
  ...
  await assert.rejects(
    client.request("im.dialog.get", { DIALOG_ID: "chat1" }),
    {
      code: "METHOD_NOT_ALLOWED",
    },
  );
  ...
  await assert.rejects(
    client.request("im.dialog.get", { DIALOG_ID: "1" }, context),
    {
      code: "METHOD_NOT_ALLOWED",
    },
  );
...
test("webhook is sourced only from env and is masked from errors and public output", async () => {
  assert.throws(
    () => new BitrixHttpClient({ env: {}, fetchImpl: async () => {} }),
    { code: "CONFIG_MISSING" },
  );
  ...
  assert.doesNotMatch(
    maskWebhook("failure " + webhook, webhook),
    /unit-test-token/u,
  );
  assert.doesNotMatch(
    JSON.stringify(toPublicError(caught, webhook)),
    /unit-test-token/u,
  );
});
...
test("retries only network, 429, QUERY_LIMIT_EXCEEDED, and 5xx", async (t) => {
  ...
  for (const scenario of [
    {
      name: "429",
      first: jsonResponse({ error: "TOO_MANY_REQUESTS" }, { status: 429 }),
    },
    {
      name: "query limit",
      first: jsonResponse({ error: "QUERY_LIMIT_EXCEEDED" }),
    },
    {
      name: "5xx",
      first: jsonResponse({ error: "TEMPORARY" }, { status: 503 }),
    },
  ]) {
  ...
  await t.test(
    "OVERLOAD_LIMIT does not retry and returns a conservative retry window",
    async () => {
  ...
  await t.test(
    "access denial is never retried even when paired with 5xx",
    async () => {
      let calls = 0;
      const client = clientWith(async () => {
        calls += 1;
  ...
  await t.test(
    "long Retry-After is persisted without a bounded sleep or immediate retry",
    async () => {
      const nowMs = Date.UTC(2026, 6, 17, 9, 0, 0);
      const sleeps = [];
      let calls = 0;
  ...
  await t.test(
    "short Retry-After sleeps within the bound and retries once",
    async () => {
...
test("upstream JSON responses are rejected before parsing when they exceed the byte cap", async () => {
  ...
  await assert.rejects(client.request("profile"), {
    code: "BITRIX_RESPONSE_TOO_LARGE",
  });
  assert.equal(calls, 1);
});
