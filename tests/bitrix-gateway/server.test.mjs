...
test("server exposes only fixed GET endpoints and validates numeric task paths before gateway access", async () => {
  ...
  const gateway = {
    ...
    health: async () => ({
      ...
      ready: true,
      userId: "42",
      scopes: ["im", "profile", "task"],
    }),
    listTasks: async (options) => ({
      ok: true,
      userId: "42",
      tasks: [{ id: String(options.offset) }],
      total: 8,
    }),
  ...
  const list = await invoke(handler, {
    url: "/v1/tasks?status=all&limit=10&offset=7",
  });
  ...
  assert.equal(
    (await invoke(handler, { url: "/v1/tasks/active?limit=1" })).status,
    400,
  );

  assert.equal(
    (await invoke(handler, { url: "/v1/tasks/123/snapshot" })).status,
    200,
  );
  for (const url of [
    "/v1/tasks/0/snapshot",
  ...
  assert.equal(
    (await invoke(handler, { method: "POST", url: "/v1/tasks" })).status,
    405,
  );
  assert.equal(
    (await invoke(handler, { method: "POST", url: "/v1/tasks/active" })).status,
    405,
  );
  assert.equal(
    (await invoke(handler, { url: "/v1/tasks?unknown=secret" })).status,
    400,
  );
  assert.equal(
    (await invoke(handler, { url: "/v1/tasks?limit=1.5" })).status,
    400,
  );
  assert.equal(
    (await invoke(handler, { url: "/v1/tasks?offset=-1" })).status,
    400,
  );
...
test("server returns fixed error schema and masks webhook material from body and logs", async () => {
  ...
  const result = await invoke(
    createRequestHandler({ gateway, logger: (entry) => logs.push(entry) }),
    { url: "/health" },
  );
